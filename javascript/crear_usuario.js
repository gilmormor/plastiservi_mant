	...
	$("#ced_usu").focusout(function()
	{
		...
		if (( codigo == null || codigo.length == 0 || /^\s+$/.test('ced_usu') ) == false)
		{
			...
			.done(function(datos){
				...
				if(datos.exito)
				{
					$('#est_ced').val(datos.datosrepre.est_ced);
					$('#nombre').val(datos.datosrepre.est_nombres);
					$('#apellido').val(datos.datosrepre.est_apellidos);
					$('#ema_usu').val('');
					$('#cla_usu').val('');
					$('#cla_usu1').val('');
					...
					$.ajax({
						...
						{
							accion    : "consultar",
							ced_usu   : datos.datosrepre.est_ced
						},
						dataType: 'json',
						encode	: true
					})
